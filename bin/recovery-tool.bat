...
@rem ##########################################################################
@rem
@rem  recovery-tool startup script for Windows
@rem
@rem ##########################################################################
...
@rem Add default JVM options here. You can also use JAVA_OPTS and RECOVERY_TOOL_OPTS to pass JVM options to this script.
...
@rem Execute recovery-tool
"%JAVA_EXE%" %DEFAULT_JVM_OPTS% %JAVA_OPTS% %RECOVERY_TOOL_OPTS%  -classpath "%CLASSPATH%" org.apache.aurora.scheduler.storage.durability.RecoveryTool %CMD_LINE_ARGS%
...
:fail
rem Set variable RECOVERY_TOOL_EXIT_CONSOLE if you need the _script_ return code instead of
rem the _cmd.exe /c_ return code!
if  not "" == "%RECOVERY_TOOL_EXIT_CONSOLE%" exit 1
exit /b 1
